...
namespace Reporting.API
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            {
                x.AddConsumer<HotelEventConsumer>();
                
                x.UsingRabbitMq((context, cfg) =>
                {
                    cfg.Host(Configuration.GetConnectionString("RabbitMQ"));

                    cfg.ReceiveEndpoint(RabbitMQSettingsConst.ReportHotelCreatedEventQueueName, e =>
                    {
                        e.ConfigureConsumer<HotelEventConsumer>(context);
                    });
                    cfg.ReceiveEndpoint(RabbitMQSettingsConst.ReportHotelDeletedEventQueueName, e =>
                    {
                        e.ConfigureConsumer<HotelEventConsumer>(context);
                    });
                });
